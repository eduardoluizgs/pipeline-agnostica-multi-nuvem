import os
import logging

from storages.storage_interface import IStorage


class DiskStorage(IStorage):

    def __init__(self):
        pass

    def write(self, content: bytes, file_name: str, file_subpath: str = None) -> None:
        logging.info(f'Escrevendo arquivo para o DISCO: {os.path.join(os.getcwd(), file_subpath, file_name)}')

    def read(self, file_name: str, file_subpath: str = None) -> bytes:
        logging.info(f'Lendo arquivo a partir do DISCO: {os.path.join(os.getcwd(), file_subpath, file_name)}')
